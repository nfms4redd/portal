define([ "jquery", "message-bus", "layout", "customization", "i18n", "jquery-ui", "layer-list" ], function($, bus, layout, customization, i18n, ui, layerList) {
	...
	var allLayersButton = $('<input type="radio" id="show_all_layers" name="layer_list_selector" checked="checked"></input><label for="show_all_layers">' + i18n.layers + '</label>');
	var activeLayersbutton = $('<input type="radio" id="show_active_layers" name="layer_list_selector"></input><label for="show_active_layers">' + i18n.selected_layers + '</label>');
